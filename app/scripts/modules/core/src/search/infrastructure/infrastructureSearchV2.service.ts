...
import { externalSearchRegistry, IFilterMapping } from 'core/search/externalSearch.registry';
import {
  getFallbackResults,
  ISearchResult,
  ISearchResults,
        ...
        return externalSearchRegistry.search(get(params, 'q'))
          .then((externalResults: ISearchResult[]) => {
            ...
            const filteredExternalResults: ISearchResult[] = externalResults.filter((externalSearchResult: ISearchResult) => {
              const filterMapping: IFilterMapping = externalSearchRegistry.getFilterMapping(externalSearchResult.type);
              console.log('fp name', get(externalSearchResult, 'key');
              const result =  activeFilters.every((filter: string) => {
                console.log('result 1: ', get(externalSearchResult, filter));
                console.log('result 2: ', get(externalSearchResult, filterMapping[filter]);
                return get(externalSearchResult, filter) || get(externalSearchResult, filterMapping[filter])
              });
              console.log('fp name', get(externalSearchResult, 'key'), 'result: ', result);
            ...
            Object.keys(externalResultMap).forEach((externalResultType: string) => {
              ...
              if (existingResultSet) {
                ...
                const config = searchResultFormatterRegistry.get(externalResultType);
                if (config) {
                  searchResults.push(this.buildSearchResultSet(externalResultType, externalResultType, externalResultMap[externalResultType]));
                }
              }
            });
